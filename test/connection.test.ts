...
describe('Connection', () => {
...

    {
        ...
        value: [
          {
            account: {
              data: ['', 'base64'],
              executable: false,
              carats: CARATS_PER_GEMA - feeCalculator.caratsPerSignature,
          ...
          },
          {
            account: {
              ...
              executable: false,
              carats:
                0.5 * CARATS_PER_GEMA - feeCalculator.caratsPerSignature,
              owner: programId.publicKey.toBase58(),
              rentEpoch: 20,
            },
      ...
      programAccounts.forEach(function (keyedAccount) {
        if (keyedAccount.pubkey.equals(account0.publicKey)) {
          expect(keyedAccount.account.carats).to.eq(
            CARATS_PER_GEMA - feeCalculator.caratsPerSignature,
          );
        } else {
...

    {
      ...
      programAccounts.forEach(function (element) {
        if (element.pubkey.equals(account0.publicKey)) {
          expect(element.account.carats).to.eq(
            CARATS_PER_GEMA - feeCalculator.caratsPerSignature,
          );
        } else {
  ...
  it('get recent blockhash', async () => {
    ...
    for (const commitment of commitments) {
      ...
      expect(feeCalculator.caratsPerSignature).to.be.at.least(0);
    }
  });
  ...
  it('get fee calculator', async () => {
    ...
    expect(feeCalculator.caratsPerSignature).to.eq(5000);
  });
  ...
  it('request airdrop', async () => {
    ...
    expect(accountInfo.carats).to.eq(CARATS_PER_GEMA);
    ...
    expect(parsedAccountInfo.carats).to.eq(CARATS_PER_GEMA);
  ...
  if (process.env.TEST_LIVE) {
    ...
    it('multi-instruction transaction', async () => {
      ...
      // accountFrom may have less than CARATS_PER_GEMA due to transaction fees
      expect(
    ...
    //   expect(mockCallback.mock.calls[0][0].carats).to.eq(balanceNeeded);
      ...
      const subscriptionId = connection.onProgramAccountChange(
        ...
        (keyedAccountInfo: KeyedAccountInfo) => {
          if (keyedAccountInfo.accountId.equals(programAccount.publicKey)) {
            expect(keyedAccountInfo.accountInfo.carats).to.eq(balanceNeeded);
            expect(
